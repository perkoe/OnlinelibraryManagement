    ...
    public partial class userprofile : System.Web.UI.Page
    {
        ...
        void getUserBookData()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT * from book_issue_tbl where member_id='" + TextBox1.Text.Trim() + "';", con);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                da.Fill(dt);
        ...
        {
            try
            {//check due date and compare with todays date
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    //Check your condition here
                    DateTime dt = Convert.ToDateTime(e.Row.Cells[5].Text); //holds due date
                    DateTime today = DateTime.Today;//hold today date
                    if (today > dt)
                    {//if it is then color it red
